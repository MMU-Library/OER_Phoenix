...
def _normalise_language(raw: str) -> str:
    """Normalise MARC language codes to ISO 639-1 where possible."""
    if not raw:
        return "en"
    v = str(raw).strip().lower()
    # MARC 008 usually uses 3-letter codes
    if v in ("eng", "en", "english"):
        return "en"
    if v in ("fre", "fra", "fr", "french"):
        return "fr"
    if v in ("ger", "deu", "de", "german"):
        return "de"
    if v in ("spa", "es", "spanish"):
        return "es"
    return v
...
def _normalise_resource_type(raw_type: str) -> str:
    """
    MARC feeds here are monograph-level records; treat them as books by default.
    """
    return "book"
                ...
                records.append(
                    {
                        ...
                        # only accept real http(s) URLs, never fall back to ISBN
                        "url": _normalise_url(url),
                        "description": description,
                        "author": ", ".join(authors) if authors else "",
                        "publisher": publisher,
                        "language": _normalise_language(language or "en"),
                        "resource_type": "book",
    ...
    def _parse_with_elementtree(self, content):
        ...
        try:
            ...
            for rec_el in root.findall(".//marc:record", ns):
                ...
                language = ""
                if (
                    controlfield_008 is not None
                    and controlfield_008.text
                    and len(controlfield_008.text) >= 40
                ):
